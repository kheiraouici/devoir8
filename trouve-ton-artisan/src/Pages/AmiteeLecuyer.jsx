import './FichesArtisan.css'

const AmiteeLecuyer  =()=>{
    return(
        <><h1>amitee lecuyer</h1>
        <section className="ficheartisan">
          <img src="../../assets/images/artisanat.jpg" alt="artisanat" />
            <span>nom:Amitee Lécuyer  </span>
            <span>spécialité : Couturier </span>
            <span>note :4,5  </span>
            <span>Ville :  Annecy </span>
            <span>à propos : Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                Phasellus eleifend ante sem, id volutpat massa fermentum nec. Praesent volutpat scelerisque mauris,
                 quis sollicitudin tellus sollicitudin.  </span>
            <span>Email : a.amitee@hotmail.com</span>
            <span>site web : https://lecuyer-couture.com</span>
            <span>catégorie :Fabrication</span>
            <span>top : Faux </span>

               <h4>formulaire de contact</h4>
           <form action="contact" id="formulaire">
            <p><input type="text" value="nom" /></p>
           <p> <input type="text" value="prénom"/></p>
           <p> <input type="email" name="email" id="email" value="email" /></p>
           </form>
           <button>envoyer</button>
        </section>
              
        
        </>
    )
}
export default AmiteeLecuyer